...
clusterName=$(curl --silent -u admin:passw0rd -X GET http://localhost:8080/api/v1/clusters/  | grep  cluster_name | sed -e 's,.*:.*"\(.*\)".*,\1,g')
...
function checkServiceStatus(){
        ...
        while :
        do
                if curl -i -u admin:passw0rd -H 'X-Requested-By:ambari' -X GET http://localhost:8080/api/v1/clusters/$clusterName/services/$service 2>&1 | grep "$status" > /dev/null  2>&1
                then
                        printf "\n $service is $status\n" >> $sdir/../log/logDistribute.log 2>&1
                        break
                fi
...
distribute() {
        ...
        command="curl -u admin:passw0rd -H 'X-Requested-By:ambari' -i -X PUT -d '{\"RequestInfo\": {\"context\" :\"Stop LOGSEARCH\"}, \"Body\": {\"ServiceInfo\": {\"maintenance_state\" : \"OFF\", \"state\": \"INSTALLED\"}}}' http://localhost:8080/api/v1/clusters/$clusterName/services/LOGSEARCH"
       ...
       eval $server_command >> $sdir/../log/logDistribute.log 2>&1
        ...
        command="curl -u admin:passw0rd -H 'X-Requested-By:ambari' -i -X PUT -d '{\"RequestInfo\": {\"context\" :\"Start LOGSEARCH\"}, \"Body\": {\"ServiceInfo\": {\"maintenance_state\" : \"OFF\", \"state\": \"STARTED\"}}}' http://localhost:8080/api/v1/clusters/$clusterName/services/LOGSEARCH"
        eval $command > $sdir/../log/logDistribute.log 2>&1
        sleep 10s
        ...
        printf "\n\n Distribution of Logs is completed. You can login to Log Search UI - $server:61888  for Log Analysis. Credentials for LogSearch UI: admin/passw0rd \n\n"
